...
data_dir = '/Users/yiwei403/Documents/SONA_data/Exp6/'
# -
...
if subfolder_name == "6a":
    stimuli_df = pd.read_csv(os.path.join(data_dir, "exp6a.csv"))
    label = 'picture'
    wav_label = 'nameobject'

if subfolder_name == "6c":
    stimuli_df = pd.read_csv(os.path.join(data_dir, "exp6c.csv"))
    label = 'repetition'
    wav_label = 'repobject'
...
pcibex_log = f'{subject_id}_Exp6_{subfolder_name}_pcibex_clean.xlsx'
...
print(f'MFA done! {str(len(textgrid_files))} generated out of 103 stimuli.')
...
for file in os.listdir(wavs_dir):
    ...
    if file.endswith('.TextGrid'):
        ...
        if item_code in pcibex_log_df['itemcode'].values:
            ...
            if 'obj_name' in pcibex_log_df.columns:#SP013, SP014, SP017
                target_word = pcibex_log_df.loc[pcibex_log_df['itemcode'] == item_code, 'obj_name'].values[0] 
                obj_freq = pcibex_log_df.loc[pcibex_log_df['itemcode'] == item_code, 'obj_freq'].values[0] 
            elif 'object_name' in pcibex_log_df.columns:
                target_word = pcibex_log_df.loc[pcibex_log_df['itemcode'] == item_code, 'object_name'].values[0] 
                obj_freq = pcibex_log_df.loc[pcibex_log_df['itemcode'] == item_code, 'object_freq'].values[0]
...
results.to_csv(os.path.join(data_dir,subject_id,subfolder_name,f'{subject_id}_Exp6_{subfolder_name}_adjustedSentenceOnsetTime.csv'), index=False)
# -
...
results = pd.read_csv(os.path.join(data_dir,subject_id,subfolder_name,f'{subject_id}_Exp6_{subfolder_name}_adjustedSentenceOnsetTime.csv'))
results['obj_freq']=np.log(results['obj_freq'])

scatter_conditions = ['obj_freq', 'ipnp_latency']
...
all_data['obj_freq']=np.log(all_data['obj_freq'])
